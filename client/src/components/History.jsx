         ...
         <div className='transaction-card card h-100'>
            <div className='transaction-body-card card-body'>
               ...
               <div className='card-text-container pe-2'>
                  ...
                  <p className='m-0'>Amount: {amount} ETH</p>
                  {message && (
                     <p className='message-text m-0'>Message: {message}</p>
                  )}
               </div>
               <img
                  src={gifUrl || url}
               ...
               <div className='time-container mt-n3 mx-auto py-1 px-3 text-center'>
                  <p className='time-text m-0'>{timestamp}</p>
               </div>
            </div>
         </div>
      </div>
...
const History = (props) => {
   ...
   useEffect(() => {
      ...
      return () => {
         window.removeEventListener('resize', changeComponentToTransaction)
      }
   })
   ...
   return (
      <section className='history'>
         {currentAccounts ? (
            <h3 className='history-title text-light my-5'>
               Latest Transactions
            </h3>
         ) : (
            <h3 className='history-title text-light my-5'>
         ...
         >
            <motion.div
               className='cards-container row flex-nowrap g-4 text-light'
               ref={containerInner}
               drag='x'
            ...
            >
               {[...transactions]
                  .reverse()
                  .map((transaction, index, transactionsArr) => (
                     <TransactionCard
                        key={index}
                        {...transaction}
                        arr={transactionsArr}
